...
export default function TeamSidebar({
        ...
        filtered.map(member => {
          ...
          return (
            <div key={member.id} className="px-2">
              <div className="flex items-center gap-2.5 rounded-xl px-2 py-2.5 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/60">
                {/* Avatar + status dot */}
                <div className="relative shrink-0">
                  <Avatar avatarUrl={member.avatar_url} name={member.full_name} size="sm" />
                  <span className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full ring-1 ring-white dark:ring-zinc-900 ${presenceDot(member.presence_status)}`} />
                </div>
                ...
                <button
                  ...
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-zinc-400 transition-colors hover:bg-zinc-100 hover:text-zinc-600 dark:hover:bg-zinc-700 dark:hover:text-zinc-300"
                >
    ...
    <div className="shrink-0 border-b border-zinc-100 px-3 py-2.5 dark:border-zinc-800">
      <div className="flex items-center gap-2 rounded-xl bg-zinc-100 px-3 py-2 dark:bg-zinc-800">
        ...
        <input
          ...
          className="min-w-0 flex-1 bg-transparent text-xs text-zinc-900 placeholder-zinc-400 focus:outline-none dark:text-zinc-50"
        />
        {search && (
          <button onClick={() => setSearch('')} className="shrink-0 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300">
  ...
  return (
    <>
      {/* ── Desktop sidebar ─────────────────────────────────── */}
      <div
        className="sticky top-20 hidden h-[calc(100vh-5.5rem)] w-full flex-col overflow-hidden rounded-2xl bg-white ring-1 ring-zinc-200 dark:bg-zinc-900 dark:ring-zinc-800 lg:flex"
      >
        {/* Header */}
        <div className="shrink-0 border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Team</h3>
      ...
      <button
        ...
        className="fixed bottom-4 right-4 z-30 flex h-12 w-12 items-center justify-center rounded-full bg-violet-600 text-white shadow-lg shadow-violet-600/30 transition-transform active:scale-95 lg:hidden"
      >
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          />
          <div className="absolute inset-y-0 right-0 flex w-72 flex-col bg-white shadow-2xl dark:bg-zinc-900">
            {/* Panel header */}
            <div className="flex shrink-0 items-center justify-between border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
              ...
              <button
                onClick={() => setTimeout(() => setMobileOpen(false), 80)}
                onTouchEnd={e => e.stopPropagation()}
                className="flex h-8 w-8 items-center justify-center rounded-xl text-zinc-400 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800"
              >
      ...
      {mounted && dropdown && dropdownMember && createPortal(
        <div
          ...
          className="fixed z-50 w-60 overflow-hidden rounded-xl bg-white shadow-xl ring-1 ring-zinc-200 dark:bg-zinc-800 dark:ring-zinc-700"
          onClick={e => e.stopPropagation()}
        >
          {/* Current task section */}
          <div className="border-b border-zinc-100 px-4 py-3 dark:border-zinc-700">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-zinc-400">Current task</p>
            {dropdownTask ? (
              <>
                ...
                <p className="mt-2 flex items-center gap-1 text-xs font-semibold text-zinc-500 dark:text-zinc-400">
                  <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-xs text-zinc-400">No active task</p>
            )}
          </div>
          ...
          <button
            ...
            className="flex w-full items-center gap-2.5 px-4 py-3 text-sm text-zinc-700 transition-colors hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-700"
          >
